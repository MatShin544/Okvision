document.addEventListener('DOMContentLoaded', () => {
    const targetBtns = document.querySelectorAll('.target-btn');
    const selectionScreen = document.getElementById('selection-screen');
    const queryScreen = document.getElementById('query-screen');
    const backBtn = document.getElementById('back-btn');
    const typeHidden = document.getElementById('type-hidden');
    const modoTexto = document.getElementById('modo-texto');
    const extraFields = document.getElementById('extra-name-fields');
    const ageInput = document.getElementById('age');
    const familyInput = document.getElementById('family');
    const workplaceInput = document.getElementById('workplace');
    const queryInput = document.getElementById('query');

    const form = document.getElementById('search-form');
    const mainView = document.getElementById('main-view');
    const resultsView = document.getElementById('results-view');
    const progressContainer = document.getElementById('progress-container');
    const submitBtn = document.getElementById('submit-btn');
    const statusText = document.getElementById('status-text');
    const progressBar = document.querySelector('.progress-bar');
    const terminalOutput = document.getElementById('terminal-output');
    const resetBtn = document.getElementById('reset-btn');

    targetBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            const type = btn.getAttribute('data-type');
            typeHidden.value = type;
            modoTexto.innerText = type.toUpperCase();
            
            // Lógica condicional para Nome
            if (type === 'Nome') {
                extraFields.style.display = 'block';
                ageInput.setAttribute('required', 'required');
                queryInput.placeholder = "Nome Completo (ex: Jon Doe)";
            } else {
                extraFields.style.display = 'none';
                ageInput.removeAttribute('required');
                if(type === 'Instagram') queryInput.placeholder = "@usuario";
                else if(type === 'Email') queryInput.placeholder = "exemplo@email.com";
                else queryInput.placeholder = "Insira o alvo...";
            }
            
            selectionScreen.style.display = 'none';
            queryScreen.style.display = 'block';
            queryInput.focus();
        });
    });

    backBtn.addEventListener('click', () => {
        selectionScreen.style.display = 'block';
        queryScreen.style.display = 'none';
        typeHidden.value = '';
        queryInput.value = '';
    });

    // Frases estilo Hacker / Progressão
    const phrases = [
        "Iniciando OKvision...",
        "Quebrando barreiras de proteção firewalls...",
        "Analisando requisições paralelas...",
        "Rastreando cruzamento de pegadas IP...",
        "Consultando base de dados OSINT global...",
        "Recuperando registros fragmentados via API...",
        "Cruzando informações diretas com repositórios...",
        "Quase lá, não pisque...",
        "Sincronização OK! Renderizando achados...",
        "Dados encontrados!"
    ];

    function generateMockResults(type, query, age, family, workplace) {
        let results = `bash\n[+] Iniciando busca: ${query}\n`;

        if (type === "Nome") {
            results += `[+] Sherlock: Múltiplos rastros sociais encontrados para '${query}'.\n`;
            results += `[+] Processos: 2 ocorrências encontradas (TJSP).\n`;
            results += `[+] Cruzamento Privado: Base de idade: ${age || 'N/A'} // Relação Familiar: ${family || 'N/A'}\n`;
            results += `[+] Google Dorks: site:linkedin.com '${query}' '${workplace || ''}' -> 1 possivel perfil encontrado.\n`;
        } else if (type === "Instagram") {
            results += `[+] Osintgram: Coletando ID de Instagram e telefone parcial para '${query}'.\n`;
            results += `[+] Possíveis e-mails: ${query.substring(0, 3)}...@email.com\n`;
            results += `[+] Conexões Instagram: @${query}_oficial, @${query}.dev\n`;
        } else {
            results += `[+] SpiderFoot: Buscando vazamentos de dados públicos para '${query}'...\n`;
            results += `[+] Vazamentos: 0 ocorrências críticas recentes.\n`;
        }
        
        results += `[!] Rastreamento concluído.`;
        return results;
    }

    form.addEventListener('submit', (e) => {
        e.preventDefault();
        
        const type = typeHidden.value;
        const query = queryInput.value;
        if(!type || !query) return;

        submitBtn.disabled = true;
        submitBtn.style.opacity = '0.5';
        submitBtn.innerText = "> PROCESSANDO PROTOCOLO...";
        progressContainer.style.display = 'block';
        
        let progress = 0;
        let phraseIndex = 0;
        
        const interval = setInterval(() => {
            progress += Math.random() * 5 + 2;
            if (progress >= 100) progress = 100;
            
            progressBar.style.width = progress + '%';
            
            let expectedIndex = Math.floor((progress / 100) * phrases.length);
            if (expectedIndex >= phrases.length) expectedIndex = phrases.length - 1;
            
            if (expectedIndex !== phraseIndex) {
                phraseIndex = expectedIndex;
                statusText.innerText = "> " + phrases[phraseIndex];
            }
            
            if (progress >= 100) {
                clearInterval(interval);
                setTimeout(() => {
                    // Após a barra encher, transita para tela de resultados estática no frontend
                    const resultString = generateMockResults(type, query, ageInput.value, familyInput.value, workplaceInput.value);
                    
                    mainView.style.display = 'none';
                    resultsView.style.display = 'block';
                    
                    // Efeito Typewriter
                    terminalOutput.innerText = "";
                    let i = 0;
                    function typeWriter() {
                        if (i < resultString.length) {
                            terminalOutput.innerHTML += resultString.charAt(i);
                            i++;
                            terminalOutput.scrollTop = terminalOutput.scrollHeight;
                            setTimeout(typeWriter, Math.random() * 15 + 5);
                        }
                    }
                    typeWriter();
                    
                }, 500);
            }
        }, 200);
    });

    resetBtn.addEventListener('click', () => {
        // Reset state
        mainView.style.display = 'block';
        resultsView.style.display = 'none';
        
        progressContainer.style.display = 'none';
        progressBar.style.width = '0%';
        submitBtn.disabled = false;
        submitBtn.style.opacity = '1';
        submitBtn.innerText = "> INICIAR VARREDURA [ENTER]";
        
        selectionScreen.style.display = 'block';
        queryScreen.style.display = 'none';
        form.reset();
        typeHidden.value = '';
    });
});
